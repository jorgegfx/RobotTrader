...
class IBStockQuotesDataProvider(host: String, port: Int) extends StockQuotesDataProvider, EWrapper {
  ...
  def getCurrentInfo(symbol: String): Try[StockQuote] = {
    ...
    if(clientSocket.isConnected) {
      ...
      Success(StockQuote(symbol="",company = "", open = 1.0, close = 2.0, high =3, low=4, snapshotTime = Instant.now()))
    }else{
  ...
  override def tickPrice(tickerId: Int, field: Int, price: Double, attrib: TickAttrib): Unit = {
    println("Receiving...")
    if (field eq TickType.LAST) System.out.println("Current price of AAPL: $" + price)
  ...
  override def orderStatus(orderId: Int, status: String, filled: Decimal, remaining: Decimal, avgFillPrice: Double, permId: Int, parentId: Int, lastFillPrice: Double, clientId: Int, whyHeld: String, mktCapPrice: Double): Unit= {}
  ...
  override def error(id: Int, errorCode: Int, errorMsg: String, advancedOrderRejectJson: String): Unit= {
  ...
  override def securityDefinitionOptionalParameter(reqId: Int, exchange: String, underlyingConId: Int, tradingClass: String, multiplier: String, expirations: util.Set[java.lang.String], strikes: util.Set[java.lang.Double]): Unit = {}
  ...
  override def smartComponents(reqId: Int, theMap: util.Map[Integer, util.Map.Entry[String, Character]]): Unit= {}
  ...
  override def historicalSchedule(reqId: Int, startDateTime: String, endDateTime: String, timeZone: String, sessions: util.List[HistoricalSession]): Unit= {}
