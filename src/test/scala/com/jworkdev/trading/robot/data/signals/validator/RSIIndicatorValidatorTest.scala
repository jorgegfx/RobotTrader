...
class RSIIndicatorValidatorTest extends AnyFlatSpec:
  ...
  it should "Result empty with less than Period" in:
    ...
    val stockPrices = List.fill(PERIOD-1)(now).zipWithIndex.map{ case (date: ZonedDateTime, index: Int) =>
  ...
  it should "Result non-empty with more than Period and gain" in :
    val now = ZonedDateTime.now()
    val stockPrices = List.fill(PERIOD + 100)(now).zipWithIndex.map { case (date: ZonedDateTime, index: Int) =>
